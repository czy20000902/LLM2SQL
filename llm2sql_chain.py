...
if __name__ == "__main__":
    ...
    for ex in examples:
        ...
        try:
            output = db_chain.invoke(prompt)
            # prediction = re.search(r'```sql\n(.*?)```', output['result'], re.DOTALL)
            # prediction = prediction.group(1).strip()
            prediction = output['result'].replace('\'', '').replace('[', '').replace(']', '')
    ...
    experiment = f'chain_{args.training_size}_{args.model_name}'
